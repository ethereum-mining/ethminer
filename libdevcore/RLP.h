...
public:
    ...
    template <unsigned _N>
    explicit operator FixedHash<_N>() const
    {
        return toHash<FixedHash<_N>>();
    }
    ...
    /// Converts to int of type given; if isString(), decodes as big-endian bytestream. @returns 0
    /// if not an int or string.
