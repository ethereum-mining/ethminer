...
{
    assert(sizeof(TARFileHeader) == BLK_SZ);
    ...
    {  // Stop if end of file has been reached or any error occured
        ...
        string filename(currentFileHeader.filename,
            min(sizeof(currentFileHeader.filename), strlen(currentFileHeader.filename)));
        if (filename == member)
        {
            bin.insert(bin.end(), fileData, fileData + size);
            rc = true;
            break;
        }
        delete[] fileData;
