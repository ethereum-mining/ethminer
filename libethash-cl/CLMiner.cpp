...
bool CLMiner::init(const h256& seed)
{
	...
	try
	{
		...
		try
		{
			...
			m_dag[0] = cl::Buffer(m_context, CL_MEM_READ_ONLY, dagSize / 2);
			m_dag[1] = cl::Buffer(m_context, CL_MEM_READ_ONLY, dagSize / 2);
		...
		m_searchKernel.setArg(6, m_dag[1]);
