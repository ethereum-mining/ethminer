...
void CLMiner::kick_miner() {
	//unsafe, can be competitions in case if two work will come simultaneously
	new thread([&]() {
		std::string logTag ="cl-" + std::to_string(Index()) + "_support";
		dev::setThreadName(logTag.c_str());
		uint32_t const c_zero = 0;
		WorkPackage w = work();
		if (current.header != w.header)
			...
			//simple workaround to remove thread race if the miner is making the init() and after that new work came
			//can be removed if we won't create new thread for each kick_miner call
			w = work();
