	...
	for (p2p::Node const& i: ns)
		if (!i.dead)
			ui->nodes->addItem(QString("[%1%3] %2 - ( =%5s | /%4s%6 ) - *%7 $%8")
						   ...
						   .arg(i.id == web3()->id() ? " self" : i.isOffline() ? QString::number(i.fallbackSeconds() - i.secondsSinceLastAttempted()) + "s" : " ----")
						   .arg(i.secondsSinceLastAttempted())
						   .arg(i.secondsSinceLastConnected())
						   .arg(i.lastDisconnect == NoDisconnect ? QString("") : (" | " + QString::fromStdString(reasonOf(i.lastDisconnect)) + " | " + QString::number(i.failedAttempts) + "x"))
						   .arg(i.rating)
						   .arg((int)i.idOrigin)
						   );
}
